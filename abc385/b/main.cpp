...
int main() {
    ...
    set<pair<i32, i32>> home_history;

    i32 sz = t.size();
    rep(i, sz) {
        switch (t[i]) {
            case 'L':
                if (x - 1 >= 0 and area[y][x - 1] != '#') --x;
                break;
            case 'R':
                if (x + 1 < w and area[y][x + 1] != '#') ++x;
                break;
            case 'U':
                if (y - 1 >= 0 and area[y - 1][x] != '#') --y;
                break;
            case 'D':
                if (y + 1 < h and area[y + 1][x] != '#') ++y;
                break;
        }
        if (area[y][x] == '@') home_history.emplace(y, x);
    }

    cout << y + 1 << ' ' << x + 1 << ' ' << home_history.size() << endl;
}