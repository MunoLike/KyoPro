...
int main() {
    ...
    vector<i32> recent_mi(n);
    recent_mi[0] = a[0];
    for (i32 i = 1; i < n; ++i) {
        recent_mi[i] = min(recent_mi[i - 1], a[i]);
    }

    rep(i, m) {
        i32 idx = lower_bound(all(recent_mi), b[i], [](auto l, auto r) { return l > r; }) - recent_mi.begin();
        upper_bound(all(recent_mi), 2);
